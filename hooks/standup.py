...
# Blocks Claude Code injects that nobody typed: recalled memories, CLAUDE.md, and the
# scaffolding around a slash command. Left in, they become a card titled
# "<local-command-caveat>Caveat: The messages below..." and credit a session with
# ticket ids lifted out of somebody else's work.
INJECTED = "system-reminder|local-command-caveat|local-command-stdout|command-message|command-name|command-args"
INJECTED_PAIR = re.compile(rf"<({INJECTED})>.*?</\1>", re.S)
INJECTED_OPEN = re.compile(rf"</?(?:{INJECTED})>")


def _clean(text: str) -> str:
    """Leave only what a human actually wrote.

    Whole tag pairs go first. A stored prompt may have been truncated mid-block, so
    anything from a dangling tag onwards is dropped too rather than shown raw.
    """
    text = INJECTED_PAIR.sub(" ", text or "")
    text = INJECTED_OPEN.split(text)[0]
...
def _text_parts(entry: dict) -> list[str]:
    ...
    if isinstance(content, str):
        ...
        return []
    return [c for c in (_clean(b) for b in blocks) if c]
...
def _self_check() -> None:
    ...
    # Injected context is stripped, but whatever the human typed around it survives.
    reminder = {"message": {"content": [{"type": "text", "text": "see <system-reminder>ABC-1</system-reminder>"}]}}
    assert _text_parts(reminder) == ["see"], _text_parts(reminder)
    only_injected = {"message": {"content": [{"type": "text", "text": "<system-reminder>ABC-1</system-reminder>"}]}}
    assert _text_parts(only_injected) == []
    # Ticket ids inside injected context must not be credited to this session.
    assert TICKET_RE.findall(_clean("<system-reminder>ABC-1</system-reminder>")) == []

    caveat = (
        "<local-command-caveat>Caveat: The messages below were generated by the user "
        "while running local commands.</local-command-caveat>fix the login bug"
    )
    assert _clean(caveat) == "fix the login bug", _clean(caveat)
    # A stored prompt truncated mid-block leaves a dangling tag; show nothing, not raw markup.
    assert _clean("<local-command-caveat>Caveat: The messages below were gen") == ""
    assert _clean("<command-name>/review</command-name>") == ""
    assert _clean("just plain text") == "just plain text" and _clean("") == ""
...
def summarize(transcript: Path, session_id: str = "", reason: str = "") -> dict | None:
    ...
    # A session driven purely by injected scaffolding is not work anyone did.
    if not first_ts or user_turns == 0 or _is_self_generated(first_prompt):
        return None
    if not first_prompt and not title:
        return None
    ...
    with HISTORY.open(errors="replace") as fh:
        for line in fh:
            ...
            if _is_self_generated(row.get("ask") or ""):
                continue
            row["ask"] = _clean(row.get("ask") or "")
            row["title"] = _clean(row.get("title") or "")
            if not row["ask"] and not row["title"]:
                continue
